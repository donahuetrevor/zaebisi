...
(function($) {
	...
	$.fn.fileUploader = function(config){
		...
		$.fileUploader.change = function(e){
			...
			if (imageUploadListDiv == false) {
				$("#px_display").append(display);
			} else {
				$(imageUploadListDiv).append(display);
			}
		...
		$(config.buttonClear).click(function(){
			...
			if (imageUploadListDiv == false) {
				displayElement = imageUploadListDiv;
			} else {
				displayElement = '#px_display';
			}
			
			$(imageUploadListDiv).fadeOut("slow",function(){
				$(imageUploadListDiv).html("");
				$("#pxupload_form").html("");
				itr = 0;
				px.appendForm();
				$('#px_button input').attr("disabled","disabled");
		...
		var px = {
			init: function(e){
				...
				if (config.imageUploadList == '#px_display') {
					before += '\
					<div id="px_display"></div>';
				} else {
					imageUploadListDiv = config.imageUploadList;
//					alert('changed'+imageUploadListDiv);
				}
				before += '\
